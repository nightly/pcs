...
namespace pcs {
	...
	bool Controller::HandleSequentialOperation(std::vector<std::string> topology_state,  std::vector<std::pair<std::vector<std::string>, 
		...
		std::map<TransferOperation, std::tuple<std::vector<std::string>, TopologyTransition, TransferOperation, TopologyTransition>> transfers;
		...
		for (const auto&[k, v] : transfers) {
			std::vector<std::string> state_vec = std::get<0>(v);
			...
			return HandleSequentialOperation(state_vec, plan_transitions, plan_parts); // recurse
		}
	}
