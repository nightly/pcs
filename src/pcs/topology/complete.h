...
namespace pcs {
	...
	public:
		CompleteTopology(const std::vector<nightly::LTS<std::string, ParameterizedOp>>& ltss, bool recursive=false);
	...
	private:
		...
		void CombineIterative(std::vector<std::string>& states_vec);
	};
